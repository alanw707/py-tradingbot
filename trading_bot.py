...
def main():
    ...
    global buy_price, stop_out_price, take_profit_price, price_signal
    ...
    while True:
        ...
        if last_trade:
            print(f"in-position: {in_position}, trade.price: {last_trade.price}, trade.type: {last_trade.trade_type},amount: {last_trade.amount}")
            print(f"stop-outprice: {last_trade.price * stop_loss_threshold_percentage}, take-profit: {last_trade.price * ((100 + take_profit_percentage) / 100)}")
            print("=========================================")
        ...
        if slowly_melt_up:
            price_signal = "slowly_melt_up"
            log_signal(session, current_timestamp, symbol, price_signal)
        ...
        if bull_flag_consolidation:
            price_signal ="bull_flag_consolidation"
        ...
        if detect_bull_flag_breakout(high_prices, low_prices, close_prices):
            ...
            price_signal ="bull_flag_breakout"
            log_signal(session, current_timestamp, symbol, price_signal)    

        if detect_golden_cross(short_ma, long_ma):
            print("Golden cross detected")
            price_signal = "golden_cross"
            log_signal(session, current_timestamp, symbol, price_signal)      

        if (short_ma[-1] > long_ma[-1]  and rsi < 30 and trend == 'uptrend' and adx > strong_trend_adx_threshold) or (trend == 'uptrend' and adx > strong_trend_adx_threshold and (price_signal=='golden_cross' or price_signal=='bull_flag_consolidation' or price_signal=='slowly_melt_up')):
            ...
            if not in_position:
                ...
                if amount_to_buy > 10:
                    ...
                    if order:
                        ...
                        stop_out_price =  buy_price * (stop_loss_threshold_percentage / 100)
        ...
        elif short_ma[-1] < long_ma[-1]  and rsi > 70 and trend == 'downtrend' and take_profit_price is not None and current_price >= take_profit_price:
        ...
        if buy_price and in_position and stop_out_price is not None and current_price <= stop_out_price:
            # Sell the asset immediately if price fall below stop_loss
            print(f"Stop-loss triggered at ${buy_price * (stop_loss_threshold_percentage / 100)}")
            amount_to_sell = exchange.fetch_balance()[symbol.split('/')[0]]['free']
            print(f"Balance: {amount_to_sell}")
            if amount_to_sell > 0:
                order = sell_order(exchange, symbol, amount_to_sell)
